    ...
    <main className="relative w-full overflow-x-hidden">
      ...
      <section className="perf-section-cv relative bg-[#FFFFFF] text-[#0F172A] px-4 sm:px-6 md:px-12 lg:px-20 py-24 arch-grid-light border-b border-slate-200">
        <div className="mx-auto max-w-7xl">
          ...
          {/* 4S Supply Model Header with Smooth Scroll Fade */}
          <ScrollItemFade className="max-w-4xl mb-16 pt-6">
            <div id="workflow">
          ...
          {/* 4S Framework Matrix Cards on Light Canvas with Individual Scroll Fade */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
            {[
              ...
              return (
                <ScrollItemFade key={item.step} scaleEffect>
                  <div
                    className="gpu-layer b2b-card-hover group relative p-7 rounded-2xl border border-slate-200 bg-white shadow-b2b-card-light hover:border-accent-borderHover hover:shadow-b2b-glow transition-all duration-200 flex flex-col justify-between cursor-pointer h-full"
                  >
                    <div>
          ...
          {/* Compliance Registry Section with Scroll Fade */}
          <ScrollItemFade>
      ...
      <footer className="w-full py-8 bg-[#F1F5F9] border-t border-slate-200 text-center text-xs font-tech text-slate-500">
        <div className="mx-auto max-w-7xl px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-accent-primary animate-pulse" />
            <span className="text-[#0F172A] font-bold">VERTEX.B2B</span>
            <span className="text-slate-500">• Authorised Commercial Master Distributor & Bulk Supply Partner</span>
          </div>
          <span className="text-slate-500 font-medium">DIRECT WHOLESALE DISTRIBUTION • FACTORY-TIER PRICING ACTIVE</span>
        </div>
      </footer>
    </main>
  );
